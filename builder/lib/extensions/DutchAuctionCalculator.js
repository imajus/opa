...
import { timestamp, takerTokenAmount } from './utils/types.js';
...
const dutchAuctionCalculatorWrapper = createWrapper({
  ...
  hooks: {
    [HookType.MAKER_AMOUNT]: createSchema({
      hint: 'Dutch auction configuration for maker amount calculation',
      fields: {
        startTime: {
          label: 'Start Time',
          type: timestamp,
          hint: 'Auction start timestamp (Unix timestamp)',
        },
        endTime: {
          label: 'End Time',
          type: timestamp,
          hint: 'Auction end timestamp (Unix timestamp)',
        },
        startAmount: {
          label: 'Start Amount',
          type: takerTokenAmount,
          hint: 'Starting taker amount (highest price for taker)',
        },
        endAmount: {
          label: 'End Amount',
          type: takerTokenAmount,
          hint: 'Ending taker amount (lowest price for taker)',
        },
      },
      validate: ({ startTime, endTime, startAmount, endAmount }) => {
  ...
  build(params) {
    ...
    const extraData = encodeDutchAuctionExtraData(config);
